...
def accept_connections():
    while True:
        ...
        while True:  # <--- added this loop
            length_data = recv_all(c, 10)
            if not length_data:
                break  # If there's no data, assume the connection is closed and break out of the loop
            message_length = int(length_data.strip())
            received_data = recv_all(c, message_length)
            if not received_data:
                break  # If there's no data, assume the connection is closed and break out of the loop
            parsed_data = json.loads(received_data.decode())
            data.update(parsed_data)  # Update the global 'data' dictionary
            print(data)  # Print the updated 'data' dictionary
